...
public class ContactController {
	...
	public @ResponseBody String findOne(
		...
		logger.debug("Inside ContactController /contact/id="+contactId+" GET");
		ContactT contact = contactService.findById(contactId);
		return ResponseConstructors.filterJsonForFieldAndViews(fields, view, contact);
	}

	/**
	 * This method is used to find all the Contacts with the given contact name or
	 * starting with given alphabet.
	 * 
	 * @param nameWith is the contact name.
	 * @param startsWith is the starting alphabet name.
	 * @return contacts.
	 */
	...
	public @ResponseBody ResponseEntity<String> findContactsWithName(
		...
		if (!nameWith.isEmpty()) {
			...
			contactlist = contactService.findContactsWithNameStarting(startsWith);
		} else {
			throw new DestinationException(HttpStatus.BAD_REQUEST, "Either nameWith / startsWith is required");
		}
		return new ResponseEntity<String>
			(ResponseConstructors.filterJsonForFieldAndViews(fields, view, contactlist), HttpStatus.OK);
	}
