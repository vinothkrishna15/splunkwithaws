...
//    public static final String TEAM_OPPORTUNITY_WIN_QUERY_ABOVE_FIVE_MILLIONS = "and (((digital_deal_value * (select conversion_rate from beacon_convertor_mapping_t where currency_name=OPP.deal_currency)) /  (select conversion_rate from beacon_convertor_mapping_t where currency_name = 'USD')) > 5000000)";
    
//    public static final String TEAM_OPPORTUNITY_WIN_QUERY_ABOVE_ONE_MILLION = "and (((digital_deal_value * (select conversion_rate from beacon_convertor_mapping_t where currency_name=OPP.deal_currency)) /  (select conversion_rate from beacon_convertor_mapping_t where currency_name = 'USD')) > 1000000)";
    
//    public static final String TEAM_OPPORTUNITY_QUERY_PART1 = "select sum(((OPP1.digital_deal_value * (select conversion_rate from beacon_convertor_mapping_t where currency_name=OPP1.deal_currency)) /  (select conversion_rate from beacon_convertor_mapping_t where currency_name = 'USD'))), count(*),OPP1.sales_stage_code from opportunity_t OPP1 where OPP1.opportunity_id in (select DISTINCT (OPP.opportunity_id) from opportunity_t OPP JOIN opportunity_sub_sp_link_t OSSL on OSSL.opportunity_id = OPP.opportunity_id JOIN sub_sp_mapping_t SSMT on OSSL.sub_sp = SSMT.sub_sp JOIN geography_country_mapping_t GCMT on GCMT.country = OPP.country JOIN geography_mapping_t GMT on GCMT.geography = GMT.geography JOIN customer_master_t CMT ON CMT.geography=GMT.geography JOIN iou_customer_mapping_t ICMT ON CMT.iou=ICMT.iou  JOIN revenue_customer_mapping_t RCMT ON GMT.geography=RCMT.customer_geography  and GMT.display_geography = '";
