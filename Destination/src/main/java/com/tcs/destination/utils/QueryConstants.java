...
public static final String TASK_TRGT_DT_POST_SUPERVISOR = "select distinct t.task_id, t.task_description, us.supervisor_user_id, ut.user_name, t.target_date_for_completion, "
   ...
   //public static final String OPPORTUNTIY_REOPEN_PENDING_WITH_USER_QUERY = "select OT.opportunity_name,WRT.status,WST.* from opportunity_t OT join workflow_request_t WRT on OT.opportunity_id = WRT.entity_id and WRT.entity_type_id = 1 join workflow_step_t WST on WST.request_id = WRT.request_id and WST.step_status ='PENDING' and WST.user_id = (:userId)";
	...
	public static final String BFM_PENDING_WITH_GROUP_QUERY="select WBT.opportunity_id,WRT.status,WRT.entity_Id, WST.* from workflow_bfm_t WBT join workflow_request_t WRT on WBT.opportunity_id = WRT.entity_id and WRT.entity_type_id IN (4,5,6) join workflow_step_t WST on WST.request_id = WRT.request_id and WST.step_status ='PENDING' and WST.user_id IS NULL and (WST.user_role like (:userRole))or WST.user_group like (:userGroup))";

	public static final String BFM_PENDING_WITH_GEO_GROUP_QUERY ="select WBT.opportunity_id,WRT.status,WRT.entity_Id,WST.* from workflow_bfm_t WBT join (select * from user_access_privileges_t where user_id = (:userId) and isactive='Y' and privilege_type = 'GEOGRAPHY' ) as UAP on UAP.privilege_value = (select geography from geography_country_mapping_t GCMT where GCMT.country = (select country from opportunity_t where opportunity_id=WBT.opportunity_id)) join workflow_request_t WRT on WBT.opportunity_id  = WRT.entity_id and WRT.entity_type_id IN (4,5,6) join workflow_step_t WST on WRT.request_id = WST.request_id where WST.step_status ='PENDING' and WST.user_id IS NULL and WST.user_group like (:userGroup)";
	
	public static final String BFM_PENDING_WITH_IOU_GROUP_QUERY="select WBT.opportunity_id,WRT.status,WRT.entity_Id,WST.* from workflow_bfm_t WBT join (select * from user_access_privileges_t where user_id =(:userId) and isactive='Y' and privilege_type ='IOU') as UAP on UAP.privilege_value = (select iou from customer_master_t CMT where CMT.customer_id = (select customer_id from opportunity_t where opportunity_id=WBT.opportunity_id)) join workflow_request_t WRT on WBT.opportunity_id  = WRT.entity_id and WRT.entity_type_id IN (4,5,6) join workflow_step_t WST on WRT.request_id = WST.request_id where WST.step_status ='PENDING' and WST.user_id IS NULL and WST.user_group like (:userGroup)";

	public static final String BFM_PENDING_WITH_SI_QUERY ="select WBT.opportunity_id,WRT.status,WRT.entity_Id,WST.* from workflow_bfm_t  WBT join workflow_request_t WRT on WBT.opportunity_id = WRT.entity_id and WRT.entity_type_id IN (4,5,6) join workflow_step_t WST on WRT.request_id = WST.request_id where WST.step_status ='PENDING' and WST.user_id IS NULL and (WST.user_role like (:userRole) or WST.user_group like (:userGroup))";

    public static final String BFM_PENDING_WITH_USER_QUERY ="select WBT.opportunity_id,WRT.status,WRT.entity_Id,WST.* from workflow_bfm_t WBT join workflow_request_t WRT on WBT.opportunity_id = WRT.entity_id and WRT.entity_type_id IN (4,5,6) join workflow_step_t WST on WRT.request_id = WST.request_id where WST.step_status ='PENDING' and WST.user_id = (:userId)";
