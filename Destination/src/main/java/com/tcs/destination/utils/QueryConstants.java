...
public class QueryConstants {
	...
	public static final String BID_DETAILS_TRGT_DT_QUERY = "select op.opportunity_id, op.opportunity_name, us.user_id, ut.user_name, bd.target_bid_submission_date "
			+ "from opportunity_t op, bid_details_t bd, user_general_settings_t us, user_t ut "
			...
			+ "from opportunity_t op, bid_details_t bd, opportunity_sales_support_link_t oss, user_general_settings_t us, user_t ut "
			...
			+ "UNION "
			+ "select op.opportunity_id, op.opportunity_name, us.user_id, ut.user_name, bd.target_bid_submission_date "
			+ "from opportunity_t op, bid_details_t bd, bid_office_group_owner_link_t bogo, user_general_settings_t us, user_t ut "
	...
	public static final String BID_DETAILS_TRGT_DT_POST_QUERY = "select op.opportunity_id, op.opportunity_name, op.opportunity_owner, ut.user_name, bd.target_bid_submission_date "
			+ "from opportunity_t op, bid_details_t bd, user_t ut where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id and op.opportunity_owner = ut.user_id "
			...
			+ "from opportunity_t op, bid_details_t bd, opportunity_sales_support_link_t oss, user_t ut "
			+ "where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id and bd.opportunity_id = oss.opportunity_id and oss.sales_support_owner = ut.user_id "
			+ "and CURRENT_DATE > bd.target_bid_submission_date and bd.actual_bid_submission_date is null and (CURRENT_DATE - bd.target_bid_submission_date) < :remindForDays "
			+ "UNION "
			+ " select op.opportunity_id, op.opportunity_name, bogo.bid_office_group_owner, ut.user_name, bd.target_bid_submission_date "
			+ "from opportunity_t op, bid_details_t bd, bid_office_group_owner_link_t bogo, user_t ut "
			+ "where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id and bd.bid_id = bogo.bid_id  and bogo.bid_office_group_owner = ut.user_id "
			+ "and CURRENT_DATE > bd.target_bid_submission_date and bd.actual_bid_submission_date is null and (CURRENT_DATE - bd.target_bid_submission_date) < :remindForDays ";
	...
	public static final String BID_DETAILS_OUTCOME_DT_POST_QUERY = "select op.opportunity_id, op.opportunity_name, op.opportunity_owner, ut.user_name , bd.expected_date_of_outcome "
			+ "from opportunity_t op, bid_details_t bd, user_t ut where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id and op.opportunity_owner = ut.user_id "
			+ "and CURRENT_DATE > bd.expected_date_of_outcome and (CURRENT_DATE - bd.expected_date_of_outcome) < :remindForDays "
			+ "UNION "
			...
			+ "and oss.sales_support_owner = ut.user_id and CURRENT_DATE > bd.expected_date_of_outcome and (CURRENT_DATE - bd.expected_date_of_outcome) < :remindForDays "
			+ "UNION "
			+ "select op.opportunity_id, op.opportunity_name, bogo.bid_office_group_owner, ut.user_name, bd.expected_date_of_outcome "
			+ "from opportunity_t op, bid_details_t bd, bid_office_group_owner_link_t bogo, user_t ut "
			+ "where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id and bd.bid_id = bogo.bid_id and bogo.bid_office_group_owner = ut.user_id and "
	...
	public static final String TASK_TRGT_DT_POST_QUERY = "select t.task_id, t.task_description, t.task_owner, ut.user_name, t.target_date_for_completion from task_t t, user_t ut "
			+ "where CURRENT_DATE > t.target_date_for_completion and t.task_status != 'CLOSED' and (CURRENT_DATE - t.target_date_for_completion) < :remindForDays and t.task_owner = ut.user_id "
			+ "UNION "
			...
			+ "and (CURRENT_DATE - t.target_date_for_completion) < :remindForDays";
	...
	public static final String BID_DETAILS_TRGT_DT_POST_SUPERVISOR = "select distinct op.opportunity_id, op.opportunity_name, us.supervisor_user_id, ut.user_name, bd.target_bid_submission_date, "
			+ "us.user_id, us.user_name as subordinate_name from opportunity_t op, bid_details_t bd, user_t us, user_t ut "
			+ "where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id and op.opportunity_owner = us.user_id and us.supervisor_user_id is not null and us.supervisor_user_id = ut.user_id "
			+ "and CURRENT_DATE > bd.target_bid_submission_date and bd.actual_bid_submission_date is null and (CURRENT_DATE - bd.target_bid_submission_date) < :remindForDays "
			...
			+ "from opportunity_t op, bid_details_t bd, opportunity_sales_support_link_t oss,user_t us, user_t ut "
			+ "where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id and bd.opportunity_id = oss.opportunity_id and oss.sales_support_owner = us.user_id and us.supervisor_user_id is not null "
			...
			+ "UNION ALL "
			+ "select distinct op.opportunity_id, op.opportunity_name, us.supervisor_user_id, ut.user_name, bd.target_bid_submission_date, us.user_id, us.user_name as subordinate_name "
			+ "from opportunity_t op, bid_details_t bd, bid_office_group_owner_link_t bogo, user_t us, user_t ut "
			+ "where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id and bogo.bid_office_group_owner = us.user_id and bd.bid_id = bogo.bid_id and us.supervisor_user_id is not null "
			+ "and us.supervisor_user_id = ut.user_id and CURRENT_DATE > bd.target_bid_submission_date and bd.actual_bid_submission_date is null and (CURRENT_DATE - bd.target_bid_submission_date) < :remindForDays "
			+ "and ((CURRENT_DATE - bd.target_bid_submission_date) % 7) = 0";
	...
	public static final String BID_DETAILS_OUTCOME_DT_POST_SUPERVISOR = "select distinct op.opportunity_id, op.opportunity_name, us.supervisor_user_id, ut.user_name, "
			...
			+ "from opportunity_t op, bid_details_t bd, user_t us, user_t ut where op.sales_stage_code between 2 and 8 and bd.opportunity_id = op.opportunity_id "
...
public static final String TASK_TRGT_DT_POST_SUPERVISOR = "select distinct t.task_id, t.task_description, us.supervisor_user_id, ut.user_name, t.target_date_for_completion, "
		+ "us.user_id, us.user_name as subordinate_name from task_t t, user_t us, user_t ut where t.task_owner = us.user_id and us.supervisor_user_id is not null and us.supervisor_user_id = ut.user_id "
