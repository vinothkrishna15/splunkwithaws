...
public class UserAccessPrivilegeQueryBuilder {
	...
	private static final Logger logger = LoggerFactory.getLogger(UserAccessPrivilegeQueryBuilder.class);
	 ...
	 * @param userId, queryPrefix
	 * @return String
	 * @throws Exception
	 */
	public String getUserAccessPrivilegeWhereConditionClause(String userId, HashMap<String, String> queryPrefix) 
			throws Exception {
		...
		if (userPrivilegesList != null && !userPrivilegesList.isEmpty()) {
			...
			return getWhereClauseString(privilegeGroups);
		} else {
	 ...
	 * This method initializes the buffers for each of the condition required 
	 * (SubSp, Iou, Geography, Customer) in the PrivilegeGroup.
	 * @param privilegeGroup, queryPrefix
	 */
	private void initGroupConditions(PrivilegeGroup privilegeGroup, HashMap<String, String> queryPrefix) throws Exception {
		...
		if (privilegeGroup != null && queryPrefix != null) {
			if (queryPrefix.get(PrivilegeType.GEOGRAPHY.name()) != null)
				privilegeGroup.setGeographyBuffer(geoBuffer.append(queryPrefix.get(PrivilegeType.GEOGRAPHY.name())));
			if (queryPrefix.get(PrivilegeType.SUBSP.name()) != null)
				privilegeGroup.setSubspBuffer(subspBuffer.append(queryPrefix.get(PrivilegeType.SUBSP.name())));
			if (queryPrefix.get(PrivilegeType.IOU.name()) != null)
				privilegeGroup.setIouBuffer(iouBuffer.append(queryPrefix.get(PrivilegeType.IOU.name())));
			if (queryPrefix.get(PrivilegeType.CUSTOMER.name()) != null)
				privilegeGroup.setCustomerBuffer(custBuffer.append(queryPrefix.get(PrivilegeType.CUSTOMER.name())));
		}
	}

	/**
	 * This method populates the required condition group object for a parent & its child privilege groups
	 * @param privilege, privilegeGroup
	 * @throws Exception
	 */
	private void populateConditionGroup(UserAccessPrivilegesT privilege, PrivilegeGroup privilegeGroup) throws Exception {
	 ...
	 * @param privilegeGroup, privilegeValue, customerBuffer
	 * @throws Exception
	 */
	private void handleCustomer(PrivilegeGroup privilegeGroup, String privilegeValue, StringBuffer customerBuffer) throws Exception {
	 ...
	 */
	private void replaceLastCommaWithParenthesisInQueryCondition(PrivilegeGroup privilegeGroup) throws Exception {
	...
	/**
	 * This method concatenates the parent clause and the other clauses with the required condition("and/or")
	 * @param clauseBuffer, condBuffer, joinString
	 */
	 ...
	 * @throws Exception
	 */
	private String getWhereClauseString(List<PrivilegeGroup> privilegeGroups) throws Exception {
		...
		if (privilegeGroups != null && !privilegeGroups.isEmpty()) {
			for (PrivilegeGroup privilegeGroup : privilegeGroups) {
				...
				if (privilegeGroup.getGeographyBuffer() != null)
					concatenateBuffer(clauseBuffer, privilegeGroup.getGeographyBuffer(), Constants.AND_CLAUSE);
				if (privilegeGroup.getSubspBuffer() != null)
					concatenateBuffer(clauseBuffer , privilegeGroup.getSubspBuffer(), Constants.AND_CLAUSE);
				if (privilegeGroup.getIouBuffer() != null)
					concatenateBuffer(clauseBuffer, privilegeGroup.getIouBuffer(), Constants.AND_CLAUSE);
				if (privilegeGroup.getCustomerBuffer() != null)
					concatenateBuffer(clauseBuffer, privilegeGroup.getCustomerBuffer(), Constants.AND_CLAUSE);
	...
	/**
	 * This method populates a HashMap with query where condition prefix (i.e tableName.columnName)
	 * @param geoPrefix, subspPrefix, iouPrefix, custPrefix
	 * @return HashMap<String, String>
	 */
	public HashMap<String, String> getQueryPrefixMap(String geoPrefix, String subspPrefix, String iouPrefix, String custPrefix) 
			throws Exception {
