...
public class OpportunityDownloadHelper {
    ...
    /**
     * this method populates the opportunity values and setting it in the corresponding cells of opportunity sheet
	...
	public int populateOpportunitySheet(Sheet opportunitySheet,
		...
		for(OpportunityT opportunity : items){
			...
			cellOppDesc.setCellValue(opportunity.getOpportunityDescription().trim());
		    }
		    
		    //Request Receive Date
		    Cell cellOppReqReceiveDate = row.createCell(11);
		    cellOppReqReceiveDate.setCellValue(getFormattedDate(opportunity.getOpportunityRequestReceiveDate().toString(), actualFormat, desiredFormat));
			...
			cellDigitalFlag.setCellValue(opportunity.getDigitalFlag().trim());
		    }
		    
		    //SALES STAGE CODE
		    Cell cellSSCode = row.createCell(15);
		    cellSSCode.setCellValue(opportunity.getSalesStageMappingT().getSalesStageDescription());
			...
			cellOverallDealValue.setCellValue(opportunity.getOverallDealSize());
		    }
		    ...
		    //if(dealValueFlag){
		    	if(opportunity.getOverallDealSize()!=null) {
		    		...
		    		cellOverallDealValue.setCellValue(convertCurrencyToUSD(opportunity.getDealCurrency(), opportunity.getOverallDealSize()).doubleValue());
		    	}
		    //}
			...
			cellDigitalDealValue.setCellValue(opportunity.getDigitalDealValue());
		    }
		    ...
		    //if(dealValueFlag){
		    	if(opportunity.getDigitalDealValue()!=null) {
		    		...
		    		cellDigitalDealValue.setCellValue(convertCurrencyToUSD(opportunity.getDealCurrency(), opportunity.getDigitalDealValue()).doubleValue());
		    	}
		    //}
		    
		    //OPPORTUNITY OWNER
		    Cell cellOppOwner = row.createCell(21);
		    cellOppOwner.setCellValue(opportunity.getPrimaryOwnerUser().getUserName());
			...
			cellClosureDate.setCellValue(getFormattedDate(opportunity.getDealClosureDate().toString(), actualFormat, desiredFormat));
		    }
		    
		    //ENGAGEMENT DURATION
		    if(opportunity.getEngagementDuration()!=null) {
			Cell cellEngDuration = row.createCell(37);
			cellEngDuration.setCellValue(opportunity.getEngagementDuration());
		    }
		    
		    //ENGAGEMENT START DATE
		    if(opportunity.getEngagementStartDate()!=null) {
			Cell cellEngStartDate = row.createCell(38);
			cellEngStartDate.setCellValue(getFormattedDate(opportunity.getEngagementStartDate().toString(), actualFormat, desiredFormat));
		    }
		    
		    //COMMENTS FOR WIN LOSS
		    if(opportunity.getDescriptionForWinLoss()!=null) {
			Cell cellCommentsWinLoss = row.createCell(40);
			cellCommentsWinLoss.setCellValue(opportunity.getDescriptionForWinLoss());
		    }
		    
		    //CUSTOMER NAME
		    Cell cellCustomerName = row.createCell(2);
		    cellCustomerName.setCellValue(opportunity.getCustomerMasterT().getCustomerName());
		    
		    //SubSp
		    if((opportunity.getOpportunitySubSpLinkTs()!=null)&&(!opportunity.getOpportunitySubSpLinkTs().isEmpty())){
			Cell cellSubSp = row.createCell(9);
			if(opportunity.getOpportunitySubSpLinkTs().size()==1){
			    cellSubSp.setCellValue(opportunity.getOpportunitySubSpLinkTs().get(0).getSubSp().trim());
			} else if(opportunity.getOpportunitySubSpLinkTs().size()>1){
			    cellSubSp.setCellValue(constructSubSpCell(opportunity.getOpportunitySubSpLinkTs()));
			}
		    }
		    
		    //Offering
		    if((opportunity.getOpportunityOfferingLinkTs()!=null)&&(!opportunity.getOpportunityOfferingLinkTs().isEmpty())){
			Cell cellOffering = row.createCell(10);
			if(opportunity.getOpportunityOfferingLinkTs().size()==1){
			    cellOffering.setCellValue(opportunity.getOpportunityOfferingLinkTs().get(0).getOffering().trim());
			} else if(opportunity.getOpportunityOfferingLinkTs().size()>1){
			    cellOffering.setCellValue(constructOfferingCell(opportunity.getOpportunityOfferingLinkTs()));
			}
		    }
...
//		    }
		    ...
		    //PARTNER NAME
		    if((opportunity.getOpportunityPartnerLinkTs()!=null)&&(!opportunity.getOpportunityPartnerLinkTs().isEmpty())){
			Cell cellPartner = row.createCell(25);
			if(opportunity.getOpportunityPartnerLinkTs().size()==1){
			    cellPartner.setCellValue(opportunity.getOpportunityPartnerLinkTs().get(0).getPartnerMasterT().getPartnerName().trim());
			} else if(opportunity.getOpportunityPartnerLinkTs().size()>1){
			    cellPartner.setCellValue(constructPartners(opportunity.getOpportunityPartnerLinkTs()));
			}  
		    }
		    
		    //COMPETITOR NAME
		    if((opportunity.getOpportunityCompetitorLinkTs()!=null)&&(!opportunity.getOpportunityCompetitorLinkTs().isEmpty())){
			Cell cellComp = row.createCell(26);
			if(opportunity.getOpportunityCompetitorLinkTs().size()==1){
			    cellComp.setCellValue(opportunity.getOpportunityCompetitorLinkTs().get(0).getCompetitorName().trim());
			} else if(opportunity.getOpportunityCompetitorLinkTs().size()>1){
			    cellComp.setCellValue(constructCompetitors(opportunity.getOpportunityCompetitorLinkTs()));
			}  
		    }
		    
		    //FACTORS FOR WIN LOSS
		    if((opportunity.getOpportunityWinLossFactorsTs()!=null)&&(!opportunity.getOpportunityWinLossFactorsTs().isEmpty())){
			Cell cellFactors = row.createCell(39);
			if(opportunity.getOpportunityWinLossFactorsTs().size()==1){
			    cellFactors.setCellValue(opportunity.getOpportunityWinLossFactorsTs().get(0).getWinLossFactorMappingT().getWinLossFactor().trim());
			} else if(opportunity.getOpportunityWinLossFactorsTs().size()>1){
			    cellFactors.setCellValue(constructWinLossFactors(opportunity.getOpportunityWinLossFactorsTs()));
			}  
		    }
		    
		    //BID Details - Bid Req Type
		    if((opportunity.getBidDetailsTs()!=null)&&(!opportunity.getBidDetailsTs().isEmpty())){
			
			// Req Type
			Cell cellBidReqType = row.createCell(27);
			cellBidReqType.setCellValue(opportunity.getBidDetailsTs().get(0).getBidRequestType());
			
			//Req Receive Date
			Cell cellBidReqReceiveDate = row.createCell(29);
			cellBidReqReceiveDate.setCellValue(getFormattedDate(opportunity.getBidDetailsTs().get(0).getBidRequestReceiveDate().toString(), actualFormat, desiredFormat));
			
			//Target Date
			Cell cellTargetDate = row.createCell(30);
			cellTargetDate.setCellValue(getFormattedDate(opportunity.getBidDetailsTs().get(0).getTargetBidSubmissionDate().toString(), actualFormat, desiredFormat));
			
			//Actual Date
			if(opportunity.getBidDetailsTs().get(0).getActualBidSubmissionDate()!=null){
			    Cell cellActualDate = row.createCell(31);
			    cellActualDate.setCellValue(getFormattedDate(opportunity.getBidDetailsTs().get(0).getActualBidSubmissionDate().toString(), actualFormat, desiredFormat));
			}
			
			//Expected Date
			if(opportunity.getBidDetailsTs().get(0).getExpectedDateOfOutcome()!=null){
			    Cell cellExpDate = row.createCell(32);
			    cellExpDate.setCellValue(getFormattedDate(opportunity.getBidDetailsTs().get(0).getExpectedDateOfOutcome().toString(), actualFormat, desiredFormat));
			}
			
			//Win probability
			if(opportunity.getBidDetailsTs().get(0).getWinProbability()!=null){
			    Cell cellWinProb = row.createCell(33);
			    cellWinProb.setCellValue(opportunity.getBidDetailsTs().get(0).getWinProbability().trim());
			}
			
			//Core Attributes
			if(opportunity.getBidDetailsTs().get(0).getCoreAttributesUsedForWinning()!=null){
			    Cell cellWinProb = row.createCell(34);
			    cellWinProb.setCellValue(opportunity.getBidDetailsTs().get(0).getCoreAttributesUsedForWinning().trim());
			}

		    }
		    
		    // Deal Status Remarks
		    if((opportunity.getNotesTs()!=null)&&(!opportunity.getNotesTs().isEmpty())){
			    Cell cellNotes = row.createCell(41);
			    cellNotes.setCellValue(opportunity.getNotesTs().get(0).getNotesUpdated());
		    }
    ...
    private String constructWinLossFactors(List<OpportunityWinLossFactorsT> opportunityWinLossFactorsTs) {
         ...
         */
        public int populateCustomerMasterSheet(Sheet customerMasterSheet,List<? extends CustomerMasterT> items,int rowCount) throws Exception{
      // Excluding the header, header starts with index 0
         ...
         * @param subSpSheet
         */
        public int populateSubSpSheet(Sheet subSpSheet, List<? extends SubSpMappingT> items, int rowCount) throws Exception{
         ...
         * @param partnerSheet
         */
        public int populatePartnerSheet(Sheet partnerSheet, List<? extends Object[]> items, int rowCount) throws Exception{
 // Excluding the header, header starts with index 0
    	for (Object[] ob : items) {
    	    // Create row with rowCount
         ...
         */
        public int populateContactSheets(Sheet partnerContactSheet, List<? extends ContactT> items, int rowCount) throws Exception{
         ...
         * @param currencySheet
         */
        public int populateCurrencySheet(Sheet winLossSheet, List<? extends BeaconConvertorMappingT> items, int rowCount) throws Exception{
         ...
         * @param currencySheet
         */
        public int populateWinLossSheet(Sheet currencySheet, List<? extends WinLossFactorMappingT> items, int rowCount) throws Exception{
