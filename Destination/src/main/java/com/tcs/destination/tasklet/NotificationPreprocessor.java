...
/**
 * gets all the audit details and putting it in the job context
 * 
 * @author TCS
 *
 */
...
public class NotificationPreprocessor implements Tasklet {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(NotificationPreprocessor.class);
	...
	public RepeatStatus execute(StepContribution contribution,
		...
		case OPPORTUNITY_EDIT:
			logger.info("getting audit details of opportunity");
			...
			BidDetailsT bidDetailsT = bidDetailsTRepository
					.findFirstByOpportunityIdOrderByModifiedDatetimeDesc(entityId);
			if (bidDetailsT != null) {
				auditBidOfficeGroupOwnerLinkTs = auditBidOfficeGroupOwnerLinkTRepository
						.findByOldBidIdAndNotifiedFalseOrderByCreatedModifiedDatetimeDesc(bidDetailsT
								.getBidId());
			}
		...
		case CONNECT_CREATE:
		case CONNECT_EDIT:
			logger.info("getting audit details of connect");
		...
		case TASK_CREATE:
		case TASK_EDIT:
			logger.info("getting audit details of task");
			auditTaskT = auditTaskTRepository
					.findFirstByTaskIdAndNotifiedFalseOrderByCreatedModifiedDatetimeDesc(entityId);

			auditTaskBdmsTaggedLinkTs = auditTaskBdmsTaggedLinkTRepository
					.findByOldTaskIdAndNotifiedFalseOrderByCreatedModifiedDatetimeDesc(entityId);
			break;
		default:
			break;
		}
		// putting audit details in job context
		jobContext.put(AUDIT_BID_DETAILS, auditBidDetailsT);
		jobContext.put(AUDIT_BID_OFFICE_GRP_OWNER,
				auditBidOfficeGroupOwnerLinkTs);
		jobContext.put(AUDIT_CONNECT, auditConnectT);
		jobContext.put(AUDIT_CONNECT_SEC_OWNERS,
				auditConnectSecondaryOwnerLinkTs);
		jobContext.put(AUDIT_OPP_SALES_SUPPORT,
	...
	public void setAuditTaskTRepository(
	...
	public void setBidDetailsTRepository(
