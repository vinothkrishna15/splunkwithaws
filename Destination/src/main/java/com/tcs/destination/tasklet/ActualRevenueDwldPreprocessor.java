 ...
 * 
 */
@Component("actualRevenueDwldPreprocessor")
public class ActualRevenueDwldPreprocessor implements Tasklet {

	private static final Logger logger = LoggerFactory
			.getLogger(ActualRevenueDwldPreprocessor.class);
	...
	public RepeatStatus execute(StepContribution contribution,
		...
		if (requestList == null) {
			requestList = dataProcessingRequestRepository
					.findByRequestTypeAndStatus(
							ACTUAL_REVENUE_DOWNLOAD.getType(),
							SUBMITTED.getStatus());
		}
		...
		if (CollectionUtils.isNotEmpty(requestList)) {
			...
			jobContext.put(NEXT_STEP, REVENUE_DWLD_PROCESSING);
