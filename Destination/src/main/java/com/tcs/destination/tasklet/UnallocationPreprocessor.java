...
import static com.tcs.destination.enums.JobStep.UNALLOCATION_PROCESSING;
import static com.tcs.destination.enums.RequestStatus.VERIFIED;
import static com.tcs.destination.enums.RequestType.UNALLOCATION_UPLOAD;
...
@Component("unallocationUploadPreprocessor")
public class UnallocationPreprocessor implements Tasklet {

	private static final Logger logger = LoggerFactory
			.getLogger(UnallocationPreprocessor.class);
	...
	public RepeatStatus execute(StepContribution contribution,
		...
		if (requestList == null) {
			requestList = dataProcessingRequestRepository
					.findByRequestTypeAndStatus(UNALLOCATION_UPLOAD.getType(),
							VERIFIED.getStatus());
		}
		...
		if (CollectionUtils.isNotEmpty(requestList)) {
			...
			jobContext.put(NEXT_STEP, UNALLOCATION_PROCESSING);
