...
import static com.tcs.destination.enums.JobStep.CUSTOMER_CONTACT_UPLOAD_PROCESSING;
import static com.tcs.destination.enums.JobStep.END;
import static com.tcs.destination.enums.RequestStatus.VERIFIED;
import static com.tcs.destination.enums.RequestType.CUSTOMER_CONTACT_UPLOAD;
...
/**
 * This tasklet deals with the request details for customer contact upload
 * 
 * @author bnpp
 *
 */
@Component("customerContactUploadPreprocessor")
public class CustomerContactUploadPreprocessor implements Tasklet {

	private static final Logger logger = LoggerFactory
			.getLogger(CustomerContactUploadPreprocessor.class);
	...
	public RepeatStatus execute(StepContribution contribution,
		...
		if (requestList == null) {
			requestList = dataProcessingRequestRepository
					.findByRequestTypeAndStatus(
							CUSTOMER_CONTACT_UPLOAD.getType(),
							VERIFIED.getStatus());
		}
		...
		if (CollectionUtils.isNotEmpty(requestList)) {
			...
			jobContext.put(NEXT_STEP, CUSTOMER_CONTACT_UPLOAD_PROCESSING);
