...
public class ProductDownloadService 
{
	...
	/**
	 * this method downloads the sheet Product Contacts
	 * @param oppFlag
	 * @return
	 * @throws Exception
	 */
	public InputStreamResource getProductContacts(boolean oppFlag) throws Exception 
	{
		logger.debug("Begin:Inside getProductContacts() method of ProductDownloadService"); 
		Workbook workbook = null;
		InputStreamResource inputStreamResource = null;
		try 
		{
			workbook =(XSSFWorkbook) ExcelUtils.getWorkBook(new File
					(PropertyUtil.getProperty
							(Constants.PRODUCT_CONTACT_TEMPLATE_LOCATION_PROPERTY_NAME)));

			// Populate Partner Contacts Sheet
			if(!oppFlag){
				// Populate Partner Master Sheet  
				populateProductMasterRefSheet(workbook.getSheet(Constants.PRODUCT_MASTER_REF_SHEET));
		...
		logger.debug("End:Inside getProductContacts() method of ProductDownloadService"); 
		return inputStreamResource;
	}
	 ...
	 * @param productSheet
	 */
	private void  populateProductMasterRefSheet(Sheet productSheet) 
	{
		//Get the Product Master Sheet From Workbook
		logger.debug("Begin:Inside populateProductMasterRefSheet() method of ProductDownloadService"); 
		int currentRow = 1; // Excluding the header, header starts with index 0
		...
		for(ProductMasterT product:productsList){
			...
			String productId = product.getProductId();
			if(!StringUtils.isEmpty(productId)){
			row.createCell(0).setCellValue(productId.toString());
			}
			
			String productName = product.getProductName();
			if(!StringUtils.isEmpty(productName)){
			row.createCell(1).setCellValue(productName.toString());
			}
			
			String productDescription = product.getProductDescription();
			if(!StringUtils.isEmpty(productDescription)){
			row.createCell(2).setCellValue(productDescription.toString());
			}
			
			String active = new Boolean(product.isActive()).toString();
			if(!StringUtils.isEmpty(active)){
			row.createCell(3).setCellValue(active.toString());
			}
			// Increment row counter
			currentRow++;
		}
		logger.debug("End:Inside populateProductMasterRefSheet() method of ProductDownloadService"); 
	}
