		...
		if (Constants.EntityType.contains(favorites.getEntityType())) {
			...
			case CUSTOMER:
				if(favorites.getCustomerId()==null){
					throw new DestinationException(HttpStatus.BAD_REQUEST,"Customer ID can not be empty");
				}
				break;
			case PARTNER:
				if(favorites.getPartnerId()==null){
					throw new DestinationException(HttpStatus.BAD_REQUEST,"Partner ID can not be empty");
				}
				break;
			case CONNECT:
				if(favorites.getConnectId()==null){
					throw new DestinationException(HttpStatus.BAD_REQUEST,"Connect ID can not be empty");
				}
				break;
			case OPPORTUNITY:
				if(favorites.getOpportunityId()==null){
					throw new DestinationException(HttpStatus.BAD_REQUEST,"Opportunity ID can not be empty");
				}
				break;
			case DOCUMENT:
