...
public class ProductDecider implements JobExecutionDecider {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(ProductDecider.class);
	...
	public FlowExecutionStatus decide(JobExecution jobExecution,
		...
		if (jobExecution.getExecutionContext().get(NEXT_STEP)
				.equals(JobStep.PRODUCT_PROCESSING)) {
			status = new FlowExecutionStatus(JobStep.PRODUCT_PROCESSING.name());
		} else if (jobExecution.getExecutionContext().get(NEXT_STEP)
				.equals(JobStep.PRODUCT_CONTACT_UPLOAD_PROCESSING)) {
			status = new FlowExecutionStatus(JobStep.PRODUCT_CONTACT_UPLOAD_PROCESSING.name());
		} else if (jobExecution.getExecutionContext().get(NEXT_STEP)
