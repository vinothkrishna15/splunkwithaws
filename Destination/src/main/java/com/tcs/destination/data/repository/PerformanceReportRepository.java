...
public interface PerformanceReportRepository extends
	...
	public List<Object[]> getRevenuesBySubSp(@Param("financialYear") String financialYear,
			@Param("quarter") String quarter,@Param("displayGeography") String displayGeography,@Param("geography") String geography,@Param("customerName") String customerName,@Param("iou") String iou);
	...
	@Query(value = "select Result.country, case when Result.actualRevenue is not null then Result.actualRevenue else '0.0' end as revenue from geography_mapping_t GMT "
			...
			+ "join revenue_customer_mapping_t RCMT on ARDT.finance_customer_name = RCMT.finance_customer_name and (RCMT.customer_name = ?3 or ?3 = '') "
			+ "where ARDT.financial_year = ?1 and (ARDT.quarter = ?2 or ?2 = '') "
			+ "group by ARDT.client_country,GCMT.geography order by actualRevenue desc) Result on GMT.geography = Result.geography where GMT.geography = ?6 order by revenue desc", nativeQuery = true)
