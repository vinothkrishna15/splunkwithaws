...
public interface ContactRepository extends CrudRepository<ContactT, String> {
    ...
    @Query(value = "SELECT * FROM contact_t "
			+ "WHERE UPPER(contact_name) LIKE UPPER(:term) "
	...
	@Query(value = "SELECT * FROM contact_t "
			+ "WHERE contact_id IN (SELECT DISTINCT(contact_id) FROM partner_contact_link_t WHERE partner_id IN (SELECT partner_id FROM partner_master_t WHERE UPPER(partner_name) LIKE UPPER(:term))) "
			+ "ORDER BY contact_name DESC "
			+ "LIMIT CASE WHEN :getAll THEN null ELSE 3 END", nativeQuery = true)
