...
public class PartnerSubSpDwldWriter implements ItemWriter<PartnerSubSpMappingT>,
		StepExecutionListener {

	private static final Logger logger = LoggerFactory
			.getLogger(PartnerSubSpDwldWriter.class);
	 ...
	 */
	@Override
	public void write(List<? extends PartnerSubSpMappingT> items) throws Exception {
		...
		if (rowCount == 1) {
			...
			sheet = workbook.getSheet(Constants.PARTNER_SUBSP_TEMPLATE_SHEET_NAME);
		}

		if (items != null) {
			for (PartnerSubSpMappingT partnerSubSpMappingT : items) {
				// Create row with rowCount
				Row row = sheet.createRow(rowCount);

				// Create new Cell and set cell value
				Cell cellPartnerSubSpId = row.createCell(1);
				cellPartnerSubSpId.setCellValue(partnerSubSpMappingT.getPartnerSubspMappingId().trim());

				Cell cellPartnerName = row.createCell(2);
				cellPartnerName.setCellValue(partnerSubSpMappingT.getPartnerMasterT().getPartnerName().trim());

				Cell cellSubSp = row.createCell(3);
				Integer subSpId=partnerSubSpMappingT.getSubSpId();
				SubSpMappingT  subSpMappingT=subSpRepository.findBySubSpId(subSpId);
