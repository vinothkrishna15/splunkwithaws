 ...
 * 
 */
public class PartnerProductRefDwldWriter implements ItemWriter<ProductMasterT>,
		StepExecutionListener {
	
	private static final Logger logger = LoggerFactory.getLogger(PartnerProductRefDwldWriter.class);
	...
	public void write(List<? extends ProductMasterT> items) throws Exception {
		...
		if (rowCount == 1) {
			...
			sheet = workbook.getSheet(Constants.PARTNER_TEMPLATE_PRODUCT_REF_SHEET_NAME);
		}
		...
		if (items != null) {
			for (ProductMasterT productT : items) {
				...
				Cell cellProductDescription = row.createCell(1);
				cellProductDescription.setCellValue(productT
						.getProductDescription());

				Cell cellActive = row.createCell(2);
