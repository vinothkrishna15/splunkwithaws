...
public class UserDwldGeoUniqWriter implements ItemWriter<GeographyMappingT>,
		StepExecutionListener {

	private static final Logger logger = LoggerFactory
			.getLogger(UserDwldGeoUniqWriter.class);
	 ...
	 */
	@Override
	public void write(List<? extends GeographyMappingT> items) throws Exception {
		...
		if(items!=null) {
			for (GeographyMappingT geoRecord : items) {
				// Create row with rowCount
				Row row = sheet.getRow(rowCount);

				String timeZoneCode = geoRecord.getGeography();
				ExcelUtils.createCell(timeZoneCode,row,3);
